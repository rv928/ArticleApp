...
class CoreWebserviceManager {
    ...
    func GetWeb_Service(_ url:String,authToken:String,httpMethod:String,parameters:[String:AnyObject]?,completionHandler: @escaping (Data?,URLResponse?,Error?) -> Void) {
        
        let defaultConfigObject: URLSessionConfiguration = URLSessionConfiguration.default
        let delegateFreeSession: URLSession = URLSession(configuration: defaultConfigObject)
        let url: URL = URL(string: "\(url)")!
        var urlRequest: URLRequest = URLRequest(url: url)
        urlRequest.addValue(RequestConstant.kApplicationHeader, forHTTPHeaderField: RequestConstant.kContent)
        urlRequest.httpMethod = httpMethod
        urlRequest.timeoutInterval = 120.0
